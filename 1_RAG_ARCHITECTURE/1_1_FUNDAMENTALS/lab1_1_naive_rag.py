...
lab1_1_naive_rag.py
====================
Naive RAG pipeline: fixed-size chunking + vector search + LLM generation.
Run this end-to-end first, exactly as-is, before touching anything.
"""
...
# ---------------------------------------------------------------------------
# STEP 1: Naive fixed-size chunking -- splits by raw character count only.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# STEP 2: Embed chunks and build a FAISS vector index.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# STEP 3: Retrieve top-k chunks for a query, with similarity scores printed
# so you can SEE the retrieval confidence, not just trust it blindly.
# ---------------------------------------------------------------------------
def retrieve(query, chunks, index, model, top_k=1):
    q_emb = model.encode([query], normalize_embeddings=True)
    scores, indices = index.search(np.array(q_emb, dtype="float32"), top_k)
    results = [(chunks[i], float(scores[0][j])) for j, i in enumerate(indices[0])]
    print(f"\nQUERY: {query}")
...
# ---------------------------------------------------------------------------
# STEP 4: Generate an answer using ONLY the retrieved context.
# ---------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    results = retrieve(question, chunks, index, embed_model, top_k=1)
